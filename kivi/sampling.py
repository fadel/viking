...
def prepare_apply_fn(
    ...
    # API
    def apply_fn_ignore_first_arg(_param_mean, p, x_eval):
        p = model_unflatten(p)
        return model_apply_fn(p, x_eval)

    def linearized_apply_fn(param_mean, p, x_eval):
        def preds_at_param(p):
            return apply_fn_ignore_first_arg(None, p, x_eval)
    ...
    return apply_fn_ignore_first_arg


def make_output_sampler(apply_fn, unflatten_fn, is_linearized=False):
    apply_fn = prepare_apply_fn(apply_fn, unflatten_fn, is_linearized)
    ...
    def sample_outputs(
        ...
        def sample_output(posterior_sample):
            preds = jax.vmap(apply_fn, in_axes=(None, None, 0))(
                param_mean, posterior_sample, x_eval
            )
            return preds
        ...
        predictions = jax.vmap(sample_output)(posterior_samples)
        return predictions
